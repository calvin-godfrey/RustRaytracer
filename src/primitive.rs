...
impl Primitive {
    ...
    pub fn new_xy_rect(x0: f64, y0: f64, x1: f64, y1: f64, k: f64, mat: Arc<Material>) -> Primitive {
        Primitive::new_xy_rect_transform(x0, y0, x1, y1, k, mat, None)
    }

    pub fn new_xy_rect_transform(x0: f64, y0: f64, x1: f64, y1: f64, k: f64, mat: Arc<Material>, transform: Option<Arc<Projective3<f64>>>) -> Primitive {
        match transform {
            Some(inner_transform) => {
                ...
                Primitive::XYRect {x0, y0, x1, y1, k, mat, bounding_box: n_box, transform: Some(Arc::clone(&inner_transform))  }
            }
            None => {
                let bounding_box = Some(BoundingBox::new(Point3::new(x0, y0, k - SMALL), Point3::new(x1, y1, k + SMALL)));
                Primitive::XYRect {x0, y0, x1, y1, k, mat, bounding_box, transform: None }
            }
        }
    }

    pub fn new_xz_rect(x0: f64, z0: f64, x1: f64, z1: f64, k: f64, mat: Arc<Material>) -> Primitive {
        Primitive::new_xz_rect_transform(x0, z0, x1, z1, k, mat, None)
    }

    pub fn new_xz_rect_transform(x0: f64, z0: f64, x1: f64, z1: f64, k: f64, mat: Arc<Material>, transform: Option<Arc<Projective3<f64>>>) -> Primitive {
        match transform {
            Some(inner_transform) => {
                ...
                let bounding_box = BoundingBox::new(p1, p2);
                let n_box = Some(util::get_new_box(bounding_box, &inner_transform));
                Primitive::XZRect {x0, z0, x1, z1, k, mat, bounding_box: n_box, transform: Some(Arc::clone(&inner_transform))  }
            }
            None => {
                let bounding_box = Some(BoundingBox::new(Point3::new(x0, k - SMALL, z0), Point3::new(x1, k + SMALL, z1)));
                Primitive::XZRect {x0, z0, x1, z1, k, mat, bounding_box, transform: None }
            }
        }
    }

    pub fn new_yz_rect(y0: f64, z0: f64, y1: f64, z1: f64, k: f64, mat: Arc<Material>) -> Primitive {
        Primitive::new_yz_rect_transform(y0, z0, y1, z1, k, mat, None)
    }

    pub fn new_yz_rect_transform(y0: f64, z0: f64, y1: f64, z1: f64, k: f64, mat: Arc<Material>, transform: Option<Arc<Projective3<f64>>>) -> Primitive {
        match transform {
            Some(inner_transform) => {
                ...
                let bounding_box = BoundingBox::new(p1, p2);
                let n_box = Some(util::get_new_box(bounding_box, &inner_transform));
                Primitive::YZRect {y0, z0, y1, z1, k, mat, bounding_box: n_box, transform: Some(Arc::clone(&inner_transform))  }
            }
            None => {
                let bounding_box = Some(BoundingBox::new(Point3::new(k - SMALL, y0, z0), Point3::new(k + SMALL, y1, z1)));
                Primitive::YZRect {y0, z0, y1, z1, k, mat, bounding_box, transform: None }
            }
        }
    }
    ...
    pub fn intersects(obj: &Primitive, ray: &Ray, tmin: f64, tmax: f64) -> Option<HitRecord> {
        match obj {
            ...
            Primitive::XYRect { x0, y0, x1, y1, k, mat, bounding_box, transform } => {xy_rect_intersect(*x0, *y0, *x1, *y1, *k, mat, transform, ray, tmin, tmax)}
            Primitive::XZRect { x0, z0, x1, z1, k, mat, bounding_box, transform } => {xz_rect_intersect(*x0, *z0, *x1, *z1, *k, mat, transform, ray, tmin, tmax)}
            Primitive::YZRect { y0, z0, y1, z1, k, mat, bounding_box, transform } => {yz_rect_intersect(*y0, *z0, *y1, *z1, *k, mat, transform, ray, tmin, tmax)}
            Primitive::FlipFace { obj } => {
    ...
    pub fn get_bounding_box(obj: &Primitive, time0: f64, time1: f64) -> &Option<BoundingBox> {
        match obj {
            ...
            Primitive::XYRect { x0, y0, x1, y1, k, mat, bounding_box, transform } => {bounding_box}
            Primitive::XZRect { x0, z0, x1, z1, k, mat, bounding_box, transform } => {bounding_box}
            Primitive::YZRect { y0, z0, y1, z1, k, mat, bounding_box, transform } => {bounding_box}
