...
use crate::hittable::{BoundingBox, HitRecord};
...
impl Primitive {
        ...
        match transform {
            Some(inner_transform) => {
                ...
                Primitive::XYRect {
                    ...
                    mat_index,
                    bounding_box: n_box,
                    transform: Some(Arc::clone(&inner_transform)),
                    light_index: std::usize::MAX,
                }
            }
            None => {
                let bounding_box = Some(BoundingBox::new(
                    Point3::new(x0, y0, k - SMALL),
                    Point3::new(x1, y1, k + SMALL),
                ));
                Primitive::XYRect {
                    x0,
                    y0,
    ...
    pub fn new_xz_rect_transform(
        ...
        match transform {
            ...
            None => {
                let bounding_box = Some(BoundingBox::new(
                    Point3::new(x0, k - SMALL, z0),
                    Point3::new(x1, k + SMALL, z1),
                ));
                Primitive::XZRect {
                    x0,
                    z0,
    ...
    pub fn new_yz_rect_transform(
        ...
        match transform {
            ...
            None => {
                let bounding_box = Some(BoundingBox::new(
                    Point3::new(k - SMALL, y0, z0),
                    Point3::new(k + SMALL, y1, z1),
                ));
                Primitive::YZRect {
                    y0,
                    z0,
    ...
    pub fn intersects(index: usize, ray: &Ray, tmin: f64, tmax: f64) -> Option<HitRecord> {
        ...
        let record = match obj {
            Primitive::Sphere {
                ...
                mat_index,
                ..
            } => sphere_intersect(center, r, *mat_index, ray, tmin, tmax),
            Primitive::Triangle {
                mesh_index,
                ind,
                mat_index,
                ..
            } => all_objects.meshes[*mesh_index]
                .intersects_triangle(ray, *ind, *mat_index, tmin, tmax),
            Primitive::XYRect {
                x0,
                y0,
                x1,
                ...
                *x0, *y0, *x1, *y1, *k, *mat_index, transform, ray, tmin, tmax,
            ),
            Primitive::XZRect {
                x0,
                z0,
                ...
                *x0, *z0, *x1, *z1, *k, *mat_index, transform, ray, tmin, tmax,
            ),
            Primitive::YZRect {
                y0,
                z0,
                ...
                *y0, *z0, *y1, *z1, *k, *mat_index, transform, ray, tmin, tmax,
            ),
            Primitive::FlipFace { obj } => {
    ...
    pub fn set_light_index(&mut self, index: usize) {
        match self {
            Primitive::Sphere { light_index, .. } => {
                *light_index = index;
            }
            Primitive::Triangle { light_index, .. } => {
                *light_index = index;
            }
            Primitive::XYRect { light_index, .. } => {
                *light_index = index;
            }
            Primitive::XZRect { light_index, .. } => {
                *light_index = index;
            }
            Primitive::YZRect { light_index, .. } => {
                *light_index = index;
            }
    ...
    pub fn sample_area(&self, u: &Point2<f64>) -> (HitRecord, f64) {
        ...
        match self {
            ...
            Primitive::Triangle {
                ...
                let p1 = mesh.p[mesh.ind[*ind + 1]];
                let p2 = mesh.p[mesh.ind[*ind + 2]];
                let p: Point3<f64> = Point3::new(0f64, 0f64, 0f64)
                    + (b[0] * p0.coords + b[1] * p1.coords + (1f64 - b[0] - b[1]) * p2.coords);
                let mut record =
                    HitRecord::make_normal(p, Unit::new_normalize(Vector3::new(1f64, 1f64, 1f64)));
                if mesh.n.len() > 0 {
                    let normal: Vector3<f64> = b[0] * mesh.n[mesh.ind[*ind]]
                        + b[1] * mesh.n[mesh.ind[*ind + 1]]
                        + (1f64 - b[0] - b[1]) * mesh.n[mesh.ind[*ind + 2]];
