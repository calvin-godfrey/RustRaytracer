...
impl<'a> Visibility<'a> {
    ...
    pub fn unoccluded(&self, infinite_light: bool) -> bool {
        ...
        let new_record = crate::geometry::get_objects()
            .node
            .intersects(&ray, 0., INFINITY, 0);
...
impl HitRecord {
    pub fn new(
        ...
        let shading = Shading {
            n,
            dpdu: Unit::new_normalize(dpdu),
            dpdv: Unit::new_normalize(dpdv),
            dndu,
            dndv,
        };
        HitRecord {
    ...
    pub fn set_shading_geometry(
        &mut self,
        dpdus: Unit<Vector3<f64>>,
        dpdvs: Unit<Vector3<f64>>,
        dndus: Vector3<f64>,
        dndvs: Vector3<f64>,
        is_auth: bool,
    ) {
...
impl Mesh {
    ...
    pub fn generate_triangles(
        ...
        for index in (0..mesh.ind.len()).step_by(3) {
            ...
            let tri_box = BoundingBox::new(
                Point3::new(x_min, y_min, z_min),
                Point3::new(x_max, y_max, z_max),
            );
            let tri: Primitive = Primitive::Triangle {
                mesh_index,
                ind: index,
                bounding_box: Some(tri_box),
                light_index: std::usize::MAX,
                mat_index,
            };
            triangles.push(tri);
        }
    ...
    pub fn intersects_triangle(
        ...
        let det = e0 + e1 + e2;
        if det.abs() < SMALL / 10000. {
            // hit directly on edge; this will hit another triangle
            return None;
        }
        ...
        if determinant.abs() < SMALL / 10000. {
            // special case
        ...
        let normal: Vector3<f64> = if self.n.len() == 0 {
            // TODO: differentiate between shader/geometric normal
            dp02.cross(&dp12)
        } else {
        ...
        if self.n.len() > 0 {
            ...
            if determinant.abs() < SMALL / 1000. {
                let dn: Vector3<f64> =
                    (self.n[ind3] - self.n[ind1]).cross(&(self.n[ind2] - self.n[ind1]));
    ...
    pub fn get_uv(&self, ind: usize) -> [Vector2<f64>; 3] {
        if self.uv.len() == 0 {
            ...
            return [
                self.uv[self.ind[ind]],
                self.uv[self.ind[ind + 1]],
                self.uv[self.ind[ind + 2]],
            ];
        }
    }
...
impl BoundingBox {
    ...
    pub fn full_intersects(&self, ray: &Ray, tmin: f64, tmax: f64) -> Option<HitRecord> {
        let xy1 = Primitive::new_xy_rect(
            self.min.x, self.min.y, self.max.x, self.max.y, self.min.z, 0,
        );
        let xy2 = Primitive::new_xy_rect(
            self.min.x, self.min.y, self.max.x, self.max.y, self.max.z, 0,
        );
        let xz1 = Primitive::new_xz_rect(
            self.min.x, self.min.z, self.max.x, self.max.z, self.min.y, 0,
        );
        let xz2 = Primitive::new_xz_rect(
            self.min.x, self.min.z, self.max.x, self.max.z, self.max.y, 0,
        );
        let yz1 = Primitive::new_yz_rect(
            self.min.y, self.min.z, self.max.y, self.max.z, self.min.x, 0,
        );
        let yz2 = Primitive::new_yz_rect(
            self.min.y, self.min.z, self.max.y, self.max.z, self.max.x, 0,
        );
        let sides = vec![xy1, xy2, xz1, xz2, yz1, yz2];
        let intersects: Vec<HitRecord> = sides
            .iter()
    ...
    pub fn make_copy(bounding_box: &Option<BoundingBox>) -> BoundingBox {
        match bounding_box {
            Some(x) => BoundingBox::new(x.min.clone(), x.max.clone()),
...
impl BvhNode {
    ...
    pub fn new(
        ...
        let left_box: Option<BoundingBox> = match &final_left {
            BvhNode::Internal {
                ...
                right,
                bounding_box,
            } => Some(BoundingBox::make_new(bounding_box)),
            BvhNode::Leaf {
                index,
                bounding_box,
            } => Some(BoundingBox::make_new(bounding_box)),
        ...
        let curr_box: Option<BoundingBox> = match left_box {
            Some(x) => match right_box {
                Some(y) => Some(BoundingBox::union(&x, &y)),
                None => Some(x),
            },
            None => match right_box {
                Some(y) => Some(y),
                None => None,
            },
        };
    ...
    fn handle_two(
        ...
        let inner_bb =
            BoundingBox::make_copy(Primitive::get_bounding_box(&objs[indices[next]], t0, t1));
        let curr_bb =
            BoundingBox::make_copy(Primitive::get_bounding_box(&objs[indices[curr]], t0, t1));
        let bb = BoundingBox::union(&inner_bb, &curr_bb);
        let inner_right = BvhNode::Leaf {
            index: indices[curr],
            bounding_box: curr_bb,
        };
        let inner_left = BvhNode::Leaf {
            index: indices[next],
            bounding_box: inner_bb,
        };
...
impl Cube {
    ...
    pub fn get_sides(&self) -> Vec<Primitive> {
        let min = self.min;
        let max = self.max;
        let z0 = Primitive::new_flip_face(Box::new(Primitive::new_xy_rect_transform(
            min.x,
            min.y,
            max.x,
            max.y,
        ...
        let z1 = Primitive::new_xy_rect_transform(
            min.x,
            min.y,
            max.x,
            max.y,
        ...
        let x0 = Primitive::new_flip_face(Box::new(Primitive::new_yz_rect_transform(
            min.y,
            min.z,
            max.y,
            max.z,
        ...
        let x1 = Primitive::new_yz_rect_transform(
            min.y,
            min.z,
            max.y,
            max.z,
        ...
        let y0 = Primitive::new_flip_face(Box::new(Primitive::new_xz_rect_transform(
            min.x,
            min.z,
            max.x,
            max.z,
        ...
        let y1 = Primitive::new_xz_rect_transform(
            min.x,
            min.z,
            max.x,
            max.z,
