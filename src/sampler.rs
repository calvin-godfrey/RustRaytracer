...
fn van_der_corput(n_samples_per_pixel_sample: i32, n_pixel_samples: i64, samples: &mut [f64]) {
    ...
    for i in 0..n_pixel_samples {
        shuffle(
            &mut samples[(i * n_samples_per_pixel_sample as i64) as usize..],
...
fn sobol_2d(n_samples_per_pixel_sample: i32, n_pixel_samples: i64, samples: &mut [Point2<f64>]) {
    ...
    gray_code_sample2(
        &c_sobol[0],
        &c_sobol[1],
        n_samples_per_pixel_sample as i64 * n_pixel_samples,
        scramble,
        samples,
    );
    for i in 0..n_pixel_samples {
...
fn stratified_sample_2d(n_x: i32, n_y: i32, jitter: bool) -> Vec<Point2<f64>> {
    ...
    for y in 0..n_y {
        for x in 0..n_x {
            ...
            let p = Point2::new(
                ONE_MINUS_EPSILON.min((x as f64 + jx) * dx),
                (y as f64 + jy) * dy,
            );
...
impl Sampler {
    ...
    pub fn request_1d_array(&mut self, n: i32) {
        ...
        self.sample_array_1d.push(Vec::with_capacity(
            ((n as i64) * self.samples_per_pixel) as usize,
        ));
    }

    pub fn request_2d_array(&mut self, n: i32) {
        self.samples_2d_sizes.push(n);
        self.sample_array_2d.push(Vec::with_capacity(
            ((n as i64) * self.samples_per_pixel) as usize,
        ));
    }
    ...
    pub fn get_1d_array(&mut self, n: i32) -> Option<(&Vec<f64>, usize)> {
        ...
        let ans = (
            self.sample_array_1d.get(self.array_1d_offset).unwrap(),
            (self.current_pixel_index * (n as i64)) as usize,
        );
    ...
    pub fn get_2d_array(&mut self, n: i32) -> Option<(&Vec<Point2<f64>>, usize)> {
        ...
        let ans = (
            self.sample_array_2d.get(self.array_2d_offset).unwrap(),
            (self.current_pixel_index * (n as i64)) as usize,
        );
...
impl PixelSampler {
    ...
    pub fn get_1d(&mut self) -> f64 {
        ...
        if (self.current_1d_dimension as usize) < self.samples_1d.len() {
            let ans = self
                .samples_1d
                .get(self.current_1d_dimension as usize)
                .unwrap()
                .get(self.sampler.current_pixel_index as usize)
                .unwrap();
    ...
    pub fn get_2d(&mut self) -> Point2<f64> {
        ...
        if (self.current_2d_dimension as usize) < self.samples_2d.len() {
            let ans = self
                .samples_2d
                .get(self.current_2d_dimension as usize)
                .unwrap()
                .get(self.sampler.current_pixel_index as usize)
                .unwrap();
...
impl GlobalSampler {
    pub fn start_pixel(&mut self, p: &Point2<i32>, sampler: &Samplers) {
        ...
        self.array_end_dim = GlobalSampler::array_start_dim()
            + (self.sampler.sample_array_1d.len() + 2 * self.sampler.sample_array_2d.len()) as i32;

        for i in 0usize..self.sampler.samples_1d_sizes.len() {
            let n_samples = *self.sampler.samples_1d_sizes.get(i).unwrap() as i64
                * self.sampler.samples_per_pixel;
            for j in 0..n_samples {
                let index: i64 = sampler.get_index_for_sample(j);
                self.sampler.sample_array_1d[i as usize][j as usize] =
                    sampler.sample_dimension(index, GlobalSampler::array_start_dim() + i as i32);
            }
        }
        ...
        for i in 0usize..self.sampler.samples_2d_sizes.len() {
            let n_samples = *self.sampler.samples_2d_sizes.get(i).unwrap() as i64
                * self.sampler.samples_per_pixel;
            for j in 0..n_samples {
                let index: i64 = sampler.get_index_for_sample(j);
    ...
    pub fn start_next_sample(&mut self, sampler: &Samplers) -> bool {
        ...
        self.interval_sample_index =
            sampler.get_index_for_sample(self.sampler.current_pixel_index + 1);
        self.sampler.start_next_sample()
    }
    ...
    pub fn get_2d(&mut self, sampler: &Samplers) -> Point2<f64> {
        if self.dimension + 1 >= GlobalSampler::array_start_dim()
            && self.dimension < self.array_end_dim
        {
            self.dimension = self.array_end_dim;
        }
...
impl Clone for Samplers {
    fn clone(&self) -> Self {
        match self {
            ...
            Samplers::ZeroTwoSequenceSampler { pixel } => Samplers::ZeroTwoSequenceSampler {
                pixel: pixel.clone(),
            },
        }
    }
...
impl Samplers {
        ...
        match s {
            Samplers::StratifiedSampler {
                ...
                for i in 0usize..pixel.samples_1d.len() {
                    pixel.samples_1d[i] =
                        stratified_sample_1d((*x_samples * *y_samples) as i32, *jitter_samples);
                    shuffle(
                        &mut pixel.samples_1d[i],
                        (*x_samples * *y_samples) as i32,
                        1,
                    );
                }
                ...
                for i in 0usize..pixel.samples_2d.len() {
                    pixel.samples_2d[i] =
                        stratified_sample_2d(*x_samples as i32, *y_samples as i32, *jitter_samples);
                    shuffle(
                        &mut pixel.samples_2d[i],
                        (*x_samples * *y_samples) as i32,
                        1,
                    );
                }
                ...
                for i in 0usize..pixel.sampler.samples_1d_sizes.len() {
                    for j in 0i64..pixel.sampler.samples_per_pixel {
                        ...
                        shuffle(
                            &mut pixel.sampler.sample_array_1d[i]
                                [(j * count) as usize..((j + 1) * count) as usize],
                            count as i32,
                            1,
                        );
                    }
                ...
                for i in 0..pixel.sampler.samples_2d_sizes.len() {
                    for j in 0usize..pixel.sampler.samples_per_pixel as usize {
                        ...
                        let samples = &mut pixel.sampler.sample_array_1d[i]
                            [j * (count as usize)..(j + 1) * (count as usize)];
            ...
            Samplers::ZeroTwoSequenceSampler { pixel } => {
                ...
                for i in 0usize..pixel.samples_1d.len() {
                    van_der_corput(
                        1,
                        pixel.sampler.samples_per_pixel,
                        &mut pixel.samples_1d[i][..],
                    );
                }
                for i in 0usize..pixel.samples_2d.len() {
                ...
                for i in 0usize..pixel.sampler.samples_1d_sizes.len() {
                    van_der_corput(
                        pixel.sampler.samples_1d_sizes[i],
                        pixel.sampler.samples_per_pixel,
                        &mut pixel.sampler.sample_array_1d[i][..],
                    );
                }
                for i in 0usize..pixel.sampler.samples_2d_sizes.len() {
                    sobol_2d(
                        pixel.sampler.samples_2d_sizes[i],
                        pixel.sampler.samples_per_pixel,
                        &mut pixel.sampler.sample_array_2d[i][..],
                    );
                }
            }
        }
    ...
    */
    ...
    pub fn new_zero_two_sequence_sampler(
        samples_per_pixel: i64,
        n_sampled_dimensions: i32,
    ) -> Self {
        let pixel = PixelSampler::new(
            (samples_per_pixel as u64).next_power_of_two() as i64,
...
fn default_get_1d() -> f64 {
    0.
}
fn default_get_2d() -> Point2<f64> {
    Point2::new(0., 0.)
}
