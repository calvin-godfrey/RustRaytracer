...
impl MicrofacetDistribution {
    ...
    */
    pub fn d(mfd: &MicrofacetDistribution, wh: &Vector3<f64>) -> f64 {
        match mfd {
            MicrofacetDistribution::Beckmann { sample_visible_area, alpha_x, alpha_y } => {
    ...
    fn default_pdf(mfd: &MicrofacetDistribution, wo: &Vector3<f64>, wh: &Vector3<f64>) -> f64 {
        if MicrofacetDistribution::get_visible_area(mfd) {
            return MicrofacetDistribution::d(mfd, wh) * MicrofacetDistribution::g1(mfd, wo) * wo.dot(wh).abs() / bxdf::abs_cos_theta(wo);
        } else {
    ...
    pub fn sample_wh(mfd: &MicrofacetDistribution, wo: &Vector3<f64>, u: &Point2<f64>) -> Unit<Vector3<f64>> {
        match mfd {
            MicrofacetDistribution::Beckmann { sample_visible_area, alpha_x, alpha_y } => {
                return if !sample_visible_area {
                    ...
                    if alpha_x == alpha_y {
                        ...
                        let alpha_x2 = alpha_x * alpha_x; let alpha_y2 = alpha_y * alpha_y;
                        tan_2_theta = -log_sample / (cos_phi * cos_phi / alpha_x2 + sin_phi * sin_phi / alpha_y2);
                    }
    ...
    pub fn make_beckmann(alpha_x: f64, alpha_y: f64, samplevis: bool) -> Self {
        ...
        MicrofacetDistribution::Beckmann { sample_visible_area: samplevis, alpha_x: x, alpha_y: y }
    }
...
fn beckmann_sample(wi: &Vector3<f64>, alpha_x: f64, alpha_y: f64, u1: f64, u2: f64) -> Unit<Vector3<f64>> {
...
fn beckmann_sample_11(cos_theta_i: f64, u1: f64, u2: f64) -> Point2<f64> {
    ...
    let normalization = 1. / (1. + c + SQRT_INV_PI * tan_theta_i * (-cot_theta_i * cot_theta_i).exp());
    ...
    while it + 1 < 10 {
        ...
        let value = normalization * (1. + b + SQRT_INV_PI * tan_theta_i * (-inv_erf * inv_erf).exp()) - sample_x;
...
fn trowbridge_reitz_sample(wi: &Vector3<f64>, alpha_x: f64, alpha_y: f64, u1: f64, u2: f64) -> Unit<Vector3<f64>> {
    let wi_s = Unit::new_normalize(Vector3::new(alpha_x * wi.x, alpha_y * wi.y, wi.z));
...
fn trowbridge_reitz_sample_11(cos_theta: f64, u1: f64, u2: f64) -> Point2<f64> {
    ...
    let slope_x = if a < 0. || slope_x2 > 1. / tan_theta { slope_x1 } else { slope_x2 };
